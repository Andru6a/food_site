...
function calculator() {
  ...
  const totalCalc = document.querySelector('.calculating__result span');
  ...
  function calcTotal() {
    if (!sex || !height || !weight || !age || !ratio) {
      totalCalc.textContent = '____';
      return;
    }
    if (sex === 'female') {
      totalCalc.textContent = Math.round((447.6 + 9.2 * weight + 3.1 * height - 4.3 * age) * ratio);
    } else {
      totalCalc.textContent = Math.round(88.36 + 13.4 * weight + 4.8 * height - 5.7 * age * ratio);
    }
  }
  ;
...
function forms(formSelector, modalTimerID) {
  ...
  const message = {
    ...
    success: 'Спасибо, скоро мы перезвоним!',
...
window.addEventListener('DOMContentLoaded', () => {
  const modalTimerID = setTimeout(() => (0,_modules_modals__WEBPACK_IMPORTED_MODULE_2__.openModal)('.modal', modalTimerID), 30000);
  ...
  (0,_modules_timer__WEBPACK_IMPORTED_MODULE_3__["default"])('.timer', '2024-12-18');
